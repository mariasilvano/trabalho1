...
const registerFormValidator = async (req, res) => {
  ...
  if (!login) {
    ...
    error = { erro: 'E-mail inválido. Use o formato usuario@dominio.com', campo: 'login' };
  } else if (!senha) {
    error = { erro: 'Necessário informar uma senha', campo: 'senha' };
  } else if (!PASSWORD_REGEX.test(senha)) {
    error = { erro: 'Senha inválida. Use pelo menos 8 caracteres, incluindo letras maiúsculas, minúsculas, números e símbolos', campo: 'senha' };
  } else if (!confirmarSenha) {
    error = { erro: 'Necessário confirmar a senha', campo: 'confirmarSenha' };
  } else if (senha !== confirmarSenha) {
    error = { erro: 'As senhas não coincidem, tente novamente!', campo: 'confirmarSenha' };
  }

  if (!error) {
    const existsUser = await db.Usuario.findOne({ where: { login } });
    if (existsUser) {
      error = { erro: 'Já existe um usuário com o mesmo login', campo: 'login' };
    }
  }
